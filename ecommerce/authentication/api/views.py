    ...
    if request.method == 'POST':
        try:
            ...
            if serializer.is_valid():
                ...
                return Response({'user': user.email, 'firstname': user.firstname, 'lastname':user.lastname, 'phone':user.phone, 'type': user.type}, status=status.HTTP_201_CREATED)
            else:
    ...
    if request.method == 'POST':
        try:
            ...
            if serializer.is_valid():
                # Save user
                user = serializer.save()
                return Response({'user': user.email, 'firstname': user.firstname, 'lastname':user.lastname,'phone':user.phone,'store_Name':user.store_name, 'type': user.type }, status=status.HTTP_201_CREATED)
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
